pipeline {
  ...
  environment {
    ...
    DOCKER_HUB_USER = 'draktorio'
    GIT_REPO = 'https://github.com/draktorio/crudapp.git'
  ...
  stages {
    ...
    stage('Build Docker Images') {
      steps {
        sh 'docker build -f php.Dockerfile . -t ${DOCKER_HUB_USER}/crudbacktest:${BUILD_NUMBER}'
        sh 'docker build -f mysql.Dockerfile . -t ${DOCKER_HUB_USER}/mysqltest:${BUILD_NUMBER}'
        sh 'docker tag ${DOCKER_HUB_USER}/crudbacktest:${BUILD_NUMBER} ${DOCKER_HUB_USER}/crudbacktest:latest'
        sh 'docker tag ${DOCKER_HUB_USER}/mysqltest:${BUILD_NUMBER} ${DOCKER_HUB_USER}/mysqltest:latest'
      }
    }
      ...
      steps {
        withCredentials([usernamePassword(credentialsId: 'docker-hub-credentials', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
          sh '''
            ...
            docker push ${DOCKER_HUB_USER}/crudbacktest:${BUILD_NUMBER}
            docker push ${DOCKER_HUB_USER}/mysqltest:${BUILD_NUMBER}
            docker push ${DOCKER_HUB_USER}/crudbacktest:latest
            docker push ${DOCKER_HUB_USER}/mysqltest:latest
          '''
        }
      }
    }
    ...
    stage('Gradual Traffic Shift') {
      steps {
        sh '''
          ...
          docker service update --image ${DOCKER_HUB_USER}/crudbacktest:${BUILD_NUMBER} ${APP_NAME}_web-server --replicas 2
          docker service update --image ${DOCKER_HUB_USER}/crudbacktest:latest ${CANARY_APP_NAME}_web-server --replicas 2
          sleep 90
